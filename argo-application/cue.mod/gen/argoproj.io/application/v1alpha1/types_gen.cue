...
//timoni:generate timoni vendor crd -f https://raw.githubusercontent.com/argoproj/argo-cd/refs/heads/master/manifests/crds/application-crd.yaml
...
#Application: {
	...
	operation?: {
		...
		sync?: {
			...
			// This is typically set in a Rollback operation and is nil during
			// a Sync operation
			source?: {
				...
				helm?: {
					// APIVersions specifies the Kubernetes resource API versions to
					// pass to Helm when templating manifests. By default,
					// Argo CD uses the API versions of the target cluster. The format
					// is [group/]version/kind.
					apiVersions?: [...string]
					...
					// KubeVersion specifies the Kubernetes API version to pass to
					// Helm when templating manifests. By default, Argo CD
					// uses the Kubernetes version of the target cluster.
					kubeVersion?: string

					// Namespace is an optional namespace to template with. If left
					// empty, defaults to the app's destination namespace.
					namespace?: string
					...
					// SkipSchemaValidation skips JSON schema validation (Helm's
					// --skip-schema-validation)
					skipSchemaValidation?: bool

					// SkipTests skips test manifest installation step (Helm's
					// --skip-tests).
					skipTests?: bool
				...
				kustomize?: {
					...
					// IgnoreMissingComponents prevents kustomize from failing when
					// components do not exist locally by not appending them to
					// kustomization file
...
#ApplicationSpec: {
	...
	// to.
	// The empty string means that application belongs to the
	// 'default' project.
	project!: string
	...
	// SourceHydrator provides a way to push hydrated manifests back
	// to git before syncing them to the cluster.
	sourceHydrator?: {
		// DrySource specifies where the dry "don't repeat yourself"
		// manifest source lives.
		drySource!: {
			// Path is a directory path within the Git repository where the
			// manifests are located
			path!: string

			// RepoURL is the URL to the git repository that contains the
		...
		hydrateTo?: {
			// TargetBranch is the branch to which hydrated manifests should
			// be committed
			targetBranch!: string
		}
		...
		syncSource!: {
			// Path is a directory path within the git repository where
			// hydrated manifests should be committed to and synced
			// from. If hydrateTo is set, this is just the path from which
